...
export default function CalendarPage() {
  ...
  const tileContent = useCallback(
    ({ date, view }: { date: Date; view: string }) => {
      ...
      const hasBooked = dayBookings.some((b) => b.status?.toLowerCase() === "booked");
      const hasNoShow = dayBookings.some((b) => b.status?.toLowerCase() === "no-show");
      return (
        <div className="flex flex-col items-center gap-0.5 mt-0.5">
          ...
          <div className="flex gap-0.5">
            {hasBooked && <span className="block w-1.5 h-1.5 rounded-full bg-success" aria-label="Booked" />}
            {hasNoShow && <span className="block w-1.5 h-1.5 rounded-full bg-danger" aria-label="No-show" />}
          </div>
        </div>
      );
    },
  ...
  return (
    <div>
      <PageHeader
        title="Calendar"
        subtitle="Monthly booking overview with indicators"
        actions={
                ...
                {selectedBookings.map((b) => (
                  <div key={b.id} className="px-5 py-3 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted">{b.serviceName} · {to12(b.slot.split("-")[0])}</p>
                    </div>
